...
// project.name ("FLAC") defined in settings.gradle.kts
project.group = "org.xiph"
project.version = "1.4.3-android-r1"
    ...
    defaultConfig {
        ...
        externalNativeBuild {
            cmake {
                ...
                arguments += "-DWITH_OGG=OFF" // only needed for tests

                arguments += "-DBUILD_CXXLIBS=OFF"
                arguments += "-DBUILD_PROGRAMS=OFF"
                arguments += "-DBUILD_EXAMPLES=OFF"
                arguments += "-DBUILD_DOCS=OFF"
                arguments += "-DINSTALL_MANPAGES=OFF"
                arguments += "-DINSTALL_PKGCONFIG_MODULES=OFF"
                arguments += "-DINSTALL_CMAKE_CONFIG_MODULE=OFF"
            }
        }
    }
...
tasks.register<Zip>("prefabAar") {
    ...
    from("aar-template")
    from("${projectDir.parentFile}/include/FLAC") {
        include("**/*.h")
        into("prefab/modules/${project.name}/include/FLAC")
    }
    abis.forEach { abi ->
...
publishing {
    val githubPackagesUrl = "https://maven.pkg.github.com/jg-hot/libflac-android"
